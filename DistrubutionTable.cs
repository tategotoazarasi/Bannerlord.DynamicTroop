...
/// <summary>
///     用于根据 Armory 中的装备，为一支队伍（Party）中的各角色分配装备。 线程安全、多线程并行处理分配逻辑。
/// </summary>
public class DistrubutionTable {
	...
	///     提供只读访问，用于让外部获取分配结果。
	/// </summary>
	public ConcurrentDictionary<CharacterObject, ConcurrentBag<Equipment>> Table
	{
		get;
	} = new();

	/// <summary>
	///     重新生成（刷新）分配表。 会清空当前表，然后基于 Armory 和对应的 Party 成员重新计算并分配装备。
	/// </summary>
	public void RefreshTable() {
					...
					if (healthyNumber > 0) {
						...
						_ = Parallel.For(
							...
							_ => {
								...
								this.Table[character].Add(new Equipment());
							}
						);
					}
				}
		...
		ConcurrentDictionary<ItemObject.ItemTypeEnum, ConcurrentBag<EquipmentElement>> itemBags = new();
				...
				element => {
					switch (element.Item.ItemType) {
						...
						case ItemObject.ItemTypeEnum.Cape:
							...
							for (int i = 0; i < this._armory.GetAmount(element); i++) {
		...
		itemBags.
			...
			ForAll(
				kv => {
					...
					int i = 0;
					foreach (KeyValuePair<CharacterObject, ConcurrentBag<Equipment>> pair in this.Table) {
						...
						foreach (Equipment eq in bag) {
							...
							// 如果还想让更多角色也共享同一物品，可以在这里加特殊逻辑， 例如不递增 i；否则就是一个物品给一个角色。
						}
					}
				}
			);
	...
	/// <summary>
	///     Debug 调试输出当前分配表的情况。 每个角色会对应一批装备。
	/// </summary>
	public void DebugPrint() {
		foreach (KeyValuePair<CharacterObject, ConcurrentBag<Equipment>> kvp in this.Table) {
			...
			// 这里如果需要更详细信息，可以遍历每一个 Equipment 的每个 Slot。 先简单打印每个 Equipment 的概览。
