...
/// <summary>
///     A comparer class for comparing two ItemObjects based on their weapon properties. Implements
///     <see cref="IComparer{T}" /> to provide custom comparison logic.
/// </summary>
public class WeaponComparer : IComparer<ItemObject> {
	/// <summary>
	///     A dictionary mapping pairs of WeaponClass values to a similarity score. This is used to
	///     compare the relationship between different weapon classes and assign a predefined score
	///     based on their compatibility or similarity.
	/// </summary>
	/// <remarks>
	///     The keys in this dictionary are tuples of <see cref="WeaponClass" /> enums, and the values
	///     are floating-point numbers representing the similarity score between the two weapon classes.
	///     A higher score indicates greater similarity. Example:
	///     - (WeaponClass.OneHandedSword, WeaponClass.TwoHandedSword) -&gt; 50 This indicates a
	///     moderate similarity between one-handed and two-handed swords.
	///     - (WeaponClass.OneHandedPolearm, WeaponClass.OneHandedAxe) -&gt; 20 This indicates a lower
	///     similarity between one-handed polearms and axes.
	/// </remarks>
	private static readonly Dictionary<(WeaponClass, WeaponClass), float> ClassPairs =
		new() {
				  ...
				  { (WeaponClass.OneHandedAxe, WeaponClass.TwoHandedAxe), 50 },
				  { (WeaponClass.Mace, WeaponClass.TwoHandedMace), 50 },
				  { (WeaponClass.OneHandedPolearm, WeaponClass.OneHandedSword), 50 },
				  { (WeaponClass.OneHandedPolearm, WeaponClass.OneHandedAxe), 20 },
				  { (WeaponClass.OneHandedPolearm, WeaponClass.Mace), 20 },
				  { (WeaponClass.TwoHandedPolearm, WeaponClass.TwoHandedSword), 20 },
				  { (WeaponClass.TwoHandedPolearm, WeaponClass.TwoHandedAxe), 20 },
				  { (WeaponClass.TwoHandedPolearm, WeaponClass.TwoHandedMace), 20 },
	...
	public int Compare(ItemObject x, ItemObject y) {
		if (!x.HasWeaponComponent ||
			!y.HasWeaponComponent) {
			return 0;
		}
	...
	private float CompareWeaponDamageType(ItemObject x, ItemObject y) {
		DamageProfile profileX = new();
		DamageProfile profileY = new();
		foreach (WeaponComponentData weapon in x.Weapons) {
			if (weapon is { ThrustDamageType: not DamageTypes.Invalid, ThrustDamage: > 0, ThrustSpeed: > 0 }) {
				profileX.Thrust.Add(weapon.ThrustDamageType);
			}

			if (weapon is { SwingDamageType: not DamageTypes.Invalid, SwingDamage: > 0, SwingSpeed: > 0 }) {
		...
		float v = 0;
		if (profileX.Thrust.Count == 0 &&
			profileY.Thrust.Count == 0) {
			v += 50;
		} else if (profileX.Thrust.Count != 0 && profileY.Thrust.Count == 0 ||
				   profileX.Thrust.Count == 0 && profileY.Thrust.Count != 0) {
			v += 0;
		} else {
			v += profileX.Thrust.Intersect(profileY.Thrust).Count() * 25 / (float)profileX.Thrust.Count;
			v += profileX.Thrust.Intersect(profileY.Thrust).Count() * 25 / (float)profileY.Thrust.Count;
		}

		if (profileX.Swing.Count == 0 &&
			profileY.Swing.Count == 0) {
			v += 50;
		} else if (profileX.Swing.Count != 0 && profileY.Swing.Count == 0 ||
				   profileX.Swing.Count == 0 && profileY.Swing.Count != 0) {
			v += 0;
		} else {
			v += profileX.Swing.Intersect(profileY.Swing).Count() * 25 / (float)profileX.Swing.Count;
			v += profileX.Swing.Intersect(profileY.Swing).Count() * 25 / (float)profileY.Swing.Count;
		}
	...
	/// <param name="y"> The second ItemObject. </param>
	/// <returns>
	///     A similarity score based on the bitwise intersection of flags, normalized by the bit counts
	///     of each flag set.
	/// </returns>
	...
	/// <summary>
	///     Represents a profile of damage types for thrust and swing damage. Used to compare damage
	///     profiles of different weapons.
	/// </summary>
	private class DamageProfile {
		/// <summary>
		///     The set of damage types associated with swing attacks.
		/// </summary>
		public readonly HashSet<DamageTypes> Swing = new();

		/// <summary>
		///     The set of damage types associated with thrust attacks.
		/// </summary>
		public readonly HashSet<DamageTypes> Thrust = new();
	}
}