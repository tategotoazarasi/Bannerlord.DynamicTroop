...
public class PartyEquipmentDistributor1 {
	...
	public PartyEquipmentDistributor1(MobileParty party, ItemRoster itemRoster) {
		_party             = party;
	...
	public PartyEquipmentDistributor1(MobileParty party, IDictionary<EquipmentElement, int> equipmentToAssign) {
		_party             = party;
		_itemRoster        = null;
	...
	public PartyEquipmentDistributor1(MobileParty party, Dictionary<ItemObject, int> objectToAssign) {
		_party             = party;
		_itemRoster        = null;
		_equipmentToAssign = new Dictionary<EquipmentElement, int>(new EquipmentElementComparer());
	...
	private void AssignHorseAndHarness() {
		...
		foreach (var assignment in Assignments) {
			...
			assignment.Equipment.AddEquipmentToSlotWithoutAgent(EquipmentIndex.HorseHarness, horseAndHarness.Harness.Value);
	...
	private void AssignEquipmentType(ItemObject.ItemTypeEnum itemType) {
		...
		foreach (var assignment in Assignments) {
			if (itemType is ItemObject.ItemTypeEnum.Horse or ItemObject.ItemTypeEnum.HorseHarness && !assignment.IsMounted)
				continue;
